...
var (
	ignoreOpts = cmpopts.IgnoreUnexported(cty.Value{}, cty.Type{})
)
...
func TestGetTypeDefaults(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.Source, func(t *testing.T) {
			...
			if !cmp.Equal(test.Want, got, ignoreOpts) {
				t.Errorf("wrong result\n%s", cmp.Diff(test.Want, got, ignoreOpts))
			}
		})
	}
}
