...
Terraform aims to read data resources during the planning phase whenever
possible, but in order to achieve a correct order of operations Terraform
defers reading a data resource until the apply phase in the following
situations:
* If at least one of the given arguments is a managed resource attribute or
  other value that Terraform cannot predict until the apply step.
* If the data resource depends directly on a managed resource that itself has
  planned changes in the current plan.
* If the data resource has
  [custom conditions](#custom-condition-checks)
  and it depends _directly or indirectly_ on a managed resource that itself
  has planned changes in the current plan.

See [Data Resource Dependencies](#data-resource-dependencies) below for more
details on what it means for a data resource to depend on other objects.

If Terraform needs to defer reading a data resource until the apply phase, it
will announce that as part of the description of planned changes. Any result
attribute of such a data resource will be unknown during planning and so cannot
