...
In most cases we do not recommend including both a `data` block and a `resource` block that both represent the same object in the same configuration, because that can prevent Terraform from understanding that the `data` block result can be affected by changes in the `resource` block. However, in situations where you need to check a result of a `resource` block that the resource itself doesn't directly export, you can use a `data` block to check that object safely as long as you place the check as a direct `postcondition` of the `data` block. Associating a postcondition with a data resource tells Terraform that the `data` block is serving as a check of an object defined elsewhere, allowing Terraform to perform actions in the correct order.
...
data "aws_ebs_volume" "example" {
  ...
  # postconditions of the data resource so that Terraform can see that
  # the data resource read must wait until after any changes to the
  # managed resource have completed.
  lifecycle {
